<template>
  <div class="div-table">
    <div class="div-table-caption">
      ...
      <button type="button" class="w-10 h-10 bg-green-700 hover:bg-green-700 text-white font-bold  rounded"
        data-toggle="modal" data-target="#addGroupModal" data-bs-toggle="modal" data-bs-target="#addGroupModal">
        <font-awesome-icon class="w-7 h-7" icon="user-group" />
      </button>
    </div>
    <div v-for="contact in contacts" :key="contact">
      <ContactComponent :class="contact.id === selectedGuid ? 'highlight-row' : 'table-row'" :name="contact.name"
        :hasNewMessages="contact.hasNewMessages" :email="contact.email" :isgroup="contact.isGroup"
        :numberOfUnreadMessages="contact.numberOfUnreadMessages" :isonline="contact.isonline"
      ...
      <div class="modal-dialog relative w-auto pointer-events-none">
        <div
          ...
          <div class="modal-body relative p-4">
            <label>Enter the email address of the contact you want to add</label>
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <input v-model="nameofgroup" type="text" class="form-control" placeholder="Name of group" />
            <Multiselect v-model="value" mode="tags" valueProp="id" :close-on-select="false" placeholder="Add members"
              label="name" :searchable="true" :createTag="false" :options="groupContacts">
          ...
          <div class="modal-footer">
            <button type="button"
              ...
              data-dismiss="modal" id="groupclose">
              Close
            </button>
            <button type="button"
  ...
  methods: {
    selectContact(contact) {
      if (this.selectedGuid != contact.id) {
        ...
        const customEvent = new CustomEvent('setSelectedContactEvent', { detail: { contact: contact } });
